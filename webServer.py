...
def utilData():
    ...
    result = rrd.update(rrdfile=rrdFile,YGCT_avg=float(data['YGCT_avg']),Heap_used=float(data['Heap_used']),S0_used=float(data['S0_used']),S0_max=float(data['S0_max']),S0_ratio=float(data['S0_ratio']),S1_used=float(data['S1_used']),S1_max=float(data['S1_max']),S1_ratio=float(data['S1_ratio']),Old_used=float(data['Old_used']),Old_max=float(data['Old_max']),Old_ratio=float(data['Old_ratio']),Eden_used=float(data['Eden_used']),Eden_max=float(data['Eden_max']),Eden_ratio=float(data['Eden_ratio']),Metadata_used=float(data['Metadata_used']),Metadata_max=float(data['Metadata_max']),Metadata_ratio=float(data['Metadata_ratio']),Heap_max=float(data['Heap_max']),Heap_ratio=float(data['Heap_ratio']),YGC=float(data['YGC']),FGC=float(data['FGC']),YGCT=float(data['YGCT']),FGCT=float(data['FGCT']),GCT=float(data['GCT']),FGCT_avg=float(data['FGCT_avg']),GCT_avg=float(data['GCT_avg']))
